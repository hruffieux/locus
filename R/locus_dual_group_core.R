...
locus_dual_group_core_ <- function(Y, list_X, list_hyper, gam_vb, list_mu_beta_vb,
  ...
  with(list_hyper, { # list_init not used with the with() function to avoid
    ...
    #
    T0_inv <- 1 / t02
    sig2_rho_vb <- update_sig2_c0_vb_(G, t02) # stands for a diagonal matrix of size d with this value on the (constant) diagonal
    
    vec_sum_log_det_rho <- - d * (log(t02) + log(G + T0_inv))
      ...
      if (batch == "y") { # optimal scheme
        ...
        for (k in sample(1:d)) {
          ...
          for (g in sample(1:G)) {
            ...
            gam_vb[g, k] <- exp(-log_one_plus_exp_(pnorm(mu_theta_vb[g] + mu_rho_vb[k], lower.tail = FALSE, log.p = TRUE) - 
                                                     pnorm(mu_theta_vb[g] + mu_rho_vb[k], log.p = TRUE) -
    ...
    if (full_output) { # for internal use only
      create_named_list_(eta, eta_vb, g_sizes, gam_vb, kappa, 
                         kappa_vb, lambda, lambda_vb, list_m1_beta, list_m1_btb,
                         list_m1_btXtXb, list_sig2_beta_star, m0,  mat_x_m1, 
                         mu_rho_vb, mu_theta_vb, n0, nu, nu_vb, rs_gam, S0_inv, 
                         sig2_inv_vb, sig2_rho_vb, sig2_theta_vb, T0_inv, tau_vb,
...
elbo_dual_group_ <- function(Y, list_X, eta, eta_vb, g_sizes, gam_vb, kappa, 
  ...
  elbo_E <- e_theta_(m0, mu_theta_vb, S0_inv, sig2_theta_vb, vec_fac_st = NULL,
                     vec_sum_log_det_theta)
  
  elbo_F <- e_rho_(mu_rho_vb, n0, sig2_rho_vb, T0_inv, vec_sum_log_det_rho)
