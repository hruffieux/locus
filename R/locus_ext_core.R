locus_ext_core_ <- function(Y, X, V, list_hyper, gam_vb, mu_beta_vb, mu_c_vb,
  ...
  with(list_hyper, { # list_init not used with the with() function to avoid
    ...
    while ((!converged) & (it <= maxit)) {
      ...
      sig2_c_vb <- 1 / (colSums(V ^ 2) + (1/s2)) # vector os size r
      ...
      if (batch) { # some updates are made batch-wise
        ...
        for (l in 1:r) {
          ...
          mu_c_vb[l, ] <- sig2_c_vb[l] * crossprod(W - mat_v_mu, V[, l])
        ...
        for (k in 1:d) {
          ...
          for (j in 1:p) {
            ...
            log_part_gam_vb <-  pnorm(mat_v_mu[j, k], log.p = TRUE) + log(sig2_beta_vb) / 2 +
          ...
          vec_v_l_k <-  V %*% mu_c_vb[, k]

          for (l in 1:r) {
            vec_v_l_k <- vec_v_l_k - V[, l] * mu_c_vb[l, k]

            mu_c_vb[l, k] <- sig2_c_vb[l] * crossprod(W[, k] - vec_v_l_k, V[, l])
      ...
      lb_new <- lower_bound_ext_(Y, X, V, W, eta, gam_vb, kappa, lambda,
                                 mu_c_vb, nu, sig2_beta_vb, sig2_c_vb,
                                 sig2_inv_vb, s2, tau_vb, m1_beta, m2_beta,
    ...
    if (full_output) { # for internal use only
      create_named_list_(W, eta, gam_vb, kappa, lambda, mu_c_vb,
                         nu, sig2_beta_vb, sig2_c_vb, sig2_inv_vb, s2,
...
lower_bound_ext_ <- function(Y, X, V, W, eta, gam_vb, kappa, lambda, mu_c_vb,
                             nu, sig2_beta_vb, sig2_c_vb, sig2_inv_vb, s2,
                             tau_vb, m1_beta, m2_beta, mat_x_m1, mat_v_mu, sum_gam) {
  ...
  B <- sum(log_sig2_inv_vb * gam_vb / 2 +
             ...
             log(2*pi) / 2 - (W_2 - 2 * W * mat_v_mu + sweep(mat_v_mu^2, 1, V^2 %*% sig2_c_vb, `+`)) /2 + #V^2 %*% sig2_c_vb +
             1 / 2 * sweep(gam_vb, 2, log(sig2_beta_vb) + 1, `*`) +
             gam_vb * entropy_(1, mat_v_mu) + (1 - gam_vb) * entropy_(0, mat_v_mu) -
             gam_vb * log(gam_vb + eps) - (1 - gam_vb) * log(1 - gam_vb + eps))


  G <- sum( 1 - log(s2) + sweep( -sweep(mu_c_vb^2, 1, sig2_c_vb, `+`) / s2, 1, log(sig2_c_vb), `+`)) / 2

  H <- sum((eta - eta_vb) * log_tau_vb -
